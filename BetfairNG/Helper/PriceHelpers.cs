...
namespace BetfairNG
{
    ...
    {
        public static Lazy<decimal[]> Table = new Lazy<decimal[]>(() => new[]
        {
        ...
        }.Select(p=>(decimal)p).ToArray());

        public static bool IsValidPrice(decimal price)
        {
            return Table.Value.Contains(price);
        }

        public static decimal AddPip(decimal price)
        {
            if (!IsValidPrice(price))
                throw new ApplicationException("Invalid Price");

            int index = Array.IndexOf(Table.Value, price);
            return Table.Value[++index];
        }

        public static decimal AddPip(decimal price, int num)
        {
            if (!IsValidPrice(price))
                throw new ApplicationException("Invalid Price");

            int index = Array.IndexOf(Table.Value, price);
            return Table.Value[index + num];
        }

        public static decimal SubtractPip(decimal price)
        {
            if (!IsValidPrice(price))
                throw new ApplicationException("Invalid Price");
        ...
        public static decimal SubtractPip(decimal price, int num)
        {
            if (!IsValidPrice(price))
                throw new ApplicationException("Invalid Price");
        ...
        public static decimal RoundDownToNearestBetfairPrice(decimal price)
        {
            if (IsValidPrice(price))
                return price;
        ...
        public static decimal RoundUpToNearestBetfairPrice(decimal price)
        {
            if (IsValidPrice(price))
                return price;
        ...
        public static decimal ApplySpread(decimal price, double percentage)
        {
            if (!IsValidPrice(price))
                throw new ApplicationException("Invalid Price");

            decimal adjustedPrice = price * (decimal)percentage;
